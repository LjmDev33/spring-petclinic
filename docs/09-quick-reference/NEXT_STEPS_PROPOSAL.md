...
1. **파일 다운로드 권한 검증** ❌
   - 현재: 모든 사용자가 첨부파일 다운로드 가능
   - 필요: 비공개 게시글 첨부파일은 작성자/관리자만 다운로드

2. **게시글 수정 시 첨부파일 관리** ❌
   - 현재: 게시글 수정 시 첨부파일 관리 불가
   - 필요: 기존 파일 삭제/추가 기능
...
4. **관리자 권한 체계 강화** ❌
   - 현재: 기본적인 ROLE_ADMIN만 있음
   - 필요: 운영자 댓글 작성 권한, 모든 게시글 관리 권한
...
6. **멀티 로그인 제어** ❌
   - 현재: 단일 로그인 (1개 세션) 고정
   - 필요: SystemConfig 기반 동적 제어 (최대 5개)
...
**작성 완료**: 2025-11-26  
**최근 업데이트**: 2025-11-27 (Phase 2 완료)  
**문서 버전**: 1.1

---

# 🎉 Phase 2 완료! 다음 단계로 나아갑시다!
